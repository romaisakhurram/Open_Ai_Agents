...
calculate_agent = Agent(
    ...
    instructions="You are calculator agent, Generate a expression to calculate the sum of two numbers",
)

@cl.on_chat_start
async def on_chart_start():
    cl.user_session.set("chat history" , [])
    await cl.Message(content = "***Welcome to the Calculator Agent*** \n\n**Please tell me what you want to calculate the sums**").send()
