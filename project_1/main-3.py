...
story_agent = Agent(
    name="Funny video Agent",
    instructions="You are writer agent, Generate a story",
)

@cl.on_chat_start
async def on_chart_start():
    cl.user_session.set("chat history" , [])
    await cl.Message(content = "***Welcome to the Story Agent*** \n\n**Please tell me what you want to generate a story**").send()
